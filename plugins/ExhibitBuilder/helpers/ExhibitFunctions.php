...
function create_section_pages($v, $last_section_id, $sectionPageSql, $sectionPageTextSql, $template) {
    ...
    if ($template == 1) {
        ...
        if ($v[2] == 'to-begin-with') {
            $db->exec($sectionPageSql, array($last_section_id, '1', 'Inspiring Speeches'));
            $db->exec($sectionPageTextSql);

            $db->exec($sectionPageSql, array($last_section_id, '2', 'Interview Insights'));
            $db->exec($sectionPageTextSql);
        }
        if ($v[2] == 'plant-seeds') {
            $db->exec($sectionPageSql, array($last_section_id, '1', 'Note Sharing'));
            $db->exec($sectionPageTextSql);

            $db->exec($sectionPageSql, array($last_section_id, '2', 'Hack Ideas'));
            $db->exec($sectionPageTextSql);
        }
        if ($v[2] == 'cultivate-ideas') {
            $db->exec($sectionPageSql, array($last_section_id, '1', 'Hacking'));
            $db->exec($sectionPageTextSql);

            $db->exec($sectionPageSql, array($last_section_id, '2', 'Hack Presentation'));
            $db->exec($sectionPageTextSql);
        }
        if ($v[2] == 'blossom-thrive') {
            $db->exec($sectionPageSql, array($last_section_id, '1', 'Hack Deployment'));
            $db->exec($sectionPageTextSql);
